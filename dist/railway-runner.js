...
import { ASSISTANT_NAME, CONTAINER_MAX_OUTPUT_SIZE, CONTAINER_TIMEOUT, DATA_DIR, GROUPS_DIR, IDLE_TIMEOUT, TIMEZONE, } from './config.js';
 ...
 */
function prepareWorkspace(group, isMain) {
...
export async function runRailwayAgent(group, input, onProcess, onOutput) {
    const startTime = Date.now();
    const { groupDir, globalDir, extraDir, ipcDir, claudeDir } = prepareWorkspace(group, input.isMain);
    const agentRunnerPath = process.env.AGENT_RUNNER_PATH ||
    ...
    return new Promise((resolve) => {
        ...
        input.secretKeyNames = Object.keys(input.secrets);
        child.stdin.write(JSON.stringify(input));
        child.stdin.end();
        delete input.secrets;
        delete input.secretKeyNames;
        ...
        const killOnTimeout = () => {
            timedOut = true;
            logger.error({ group: group.name, processName }, 'Railway agent timeout, sending SIGTERM');
            child.kill('SIGTERM');
            setTimeout(() => {
                if (!child.killed) {
        ...
        child.stdout.on('data', (data) => {
            ...
            if (!stdoutTruncated) {
                ...
                if (chunk.length > remaining) {
                    ...
                    logger.warn({ group: group.name, size: stdout.length }, 'Railway agent stdout truncated');
                }
                else {
                    stdout += chunk;
                }
        ...
        child.stderr.on('data', (data) => {
            ...
            for (const line of lines) {
                if (line)
                    logger.debug({ process: group.folder }, line);
            }
            if (stderrTruncated)
                return;
        ...
        child.on('close', (code) => {
            ...
            if (timedOut) {
                if (hadStreamingOutput) {
                    logger.info({ group: group.name, processName, duration, code }, 'Railway agent timed out after output (idle cleanup)');
                    outputChain.then(() => {
            ...
            if (code !== 0) {
                logger.error({ group: group.name, code, duration, logFile }, 'Railway agent exited with error');
                resolve({
                    status: 'error',
                    result: null,
            ...
            if (onOutput) {
                outputChain.then(() => {
                    logger.info({ group: group.name, duration, newSessionId }, 'Railway agent completed (streaming mode)');
                    resolve({ status: 'success', result: null, newSessionId });
                });
                return;
            }
            ...
            try {
                ...
                logger.info({ group: group.name, duration, status: output.status }, 'Railway agent completed');
                resolve(output);
            }
            catch (err) {
                logger.error({ group: group.name, stdout, stderr, error: err }, 'Failed to parse Railway agent output');
                resolve({
                    status: 'error',
                    result: null,
        ...
        child.on('error', (err) => {
            clearTimeout(timeout);
            logger.error({ group: group.name, processName, error: err }, 'Railway agent spawn error');
            resolve({
                status: 'error',
                result: null,
...
//# sourceMappingURL=railway-runner.js.map