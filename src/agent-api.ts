...
async function notifyGroup(text: string): Promise<void> {
  if (!ECHO_TELEGRAM_TOKEN || !TELEGRAM_GROUP_CHAT_ID) return;
  try {
    await fetch(
      `https://api.telegram.org/bot${ECHO_TELEGRAM_TOKEN}/sendMessage`,
      {
        method: 'POST',
...
export function startAgentApiServer() {
  if (!ECHO_AGENT_API_TOKEN) {
    console.warn(
      '[agent-api] ECHO_AGENT_API_TOKEN not set — agent API server not started',
    );
    return;
  }
  ...
  app.post('/agent', (req, res) => {
    ...
    try {
      ...
      console.log(
        `[agent-api] Enqueued handoff task for session ${body.sessionTag}`,
      );
    } catch (err) {
